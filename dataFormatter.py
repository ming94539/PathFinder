...
class DataFormatter:
    ...
    #argument is a file object referencing 
    ...
    def read_termsFile(self, termsFile):
    ...
    def data_extraction(self):    
        ...
        for i in range(len(self.jobPosts)):  
            ...
            #Degree level
            output[self.id_list[i]]['degree_level']=self.extract_degree_lvl(self.jobPosts[i])
            #DEGREE TITLE
            output[self.id_list[i]]['degree_title']=self.extract_degree_title(self.jobPosts[i])
            #YoE
...
'''
# output_file = open("formatted.txt","a")
# input_file = open("results.txt","a+")
# input_file.seek(0)
# formatter = DataFormatter("final_keywords.txt")
# formatter.preprocessing(input_file)

# output = formatter.data_extraction()
# import pprint

# pprint.pprint(output, stream = output_file)
